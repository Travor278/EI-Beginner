...
def clip_zero_shot_classifier(clip_model, clip_preprocess, images,
    ...
    similarities = get_similarity_no_loop(text_features, image_features).t()
    pred_idxs = similarities.argmax(dim=1)
    pred_classes = [class_texts[idx] for idx in pred_idxs.tolist()]
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
...
class CLIPImageRetriever:
    ...
    def __init__(self, clip_model, clip_preprocess, images, device):
        ...
        self.image_features = clip_model.encode_image(image_inputs).float()
