...
class Unet(nn.Module):
    ...
    def cfg_forward(self, x, time, model_kwargs={}):
        ...
        ##################################################################
        cond_eps = self.forward(x, time, model_kwargs=model_kwargs)
        uncond_kwargs = copy.deepcopy(model_kwargs)
        uncond_kwargs["text_emb"] = None
        uncond_eps = self.forward(x, time, model_kwargs=uncond_kwargs)
        x = (cfg_scale + 1) * cond_eps - cfg_scale * uncond_eps
        ##################################################################
