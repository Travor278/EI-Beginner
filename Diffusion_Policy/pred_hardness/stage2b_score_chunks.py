"""
Stage 2b — Chunk-level predictive hardness scoring.

This script computes a **per-chunk** hardness score for every valid training
sample in the Push-T dataset.  It complements the existing
`scripts/stage2_score_predictive_hardness.py` which scores at the coarser
episode level.

What a "chunk" is
-----------------
Each chunk is the training window starting at global frame index obs_start:
    obs    : frames [obs_start, obs_start + obs_horizon)
    action : frames [obs_start, obs_start + pred_horizon)

The hardness of a chunk is the mean one-step L2 prediction error of the
trained state-space forward probe over the pred_horizon transition steps:
...
Outputs
-------
    <outdir>/chunk_hardness.npy          — (N_train_chunks,) float32 raw scores
    <outdir>/chunk_hardness_norm.npy     — same, min-max normalised to [0,1]
    <outdir>/chunk_hardness_meta.json    — index metadata + statistics
    <outdir>/chunk_hardness_summary.csv  — human-readable top/bottom 20 chunks
...
def main() -> None:
    ...
    # ---------------------------------------------------------------- #
    # 1. Build dataset to get train chunk indices                        #
    # ---------------------------------------------------------------- #
    print("Building train dataset index …")
    # We only need the index (obs_starts) — no hardness scores needed yet
    ...
    # ---------------------------------------------------------------- #
    # 2. Load state arrays for the probe                                 #
    # ---------------------------------------------------------------- #
    print("Loading state arrays from zarr …")
    import zarr as zarr_lib
    root = zarr_lib.open(str(args.zarr_path), mode="r")
    state  = np.asarray(root["data"]["state"][:],  dtype=np.float32)   # (N_frames, sd)
    action = np.asarray(root["data"]["action"][:], dtype=np.float32)   # (N_frames, 2)
    ...
    hardness_norm = ChunkScorer.normalise_scores(hardness_raw)  # (N,) in [0,1]
    ...
    # Human-readable CSV with top/bottom 20 chunks
    df = pd.DataFrame({
