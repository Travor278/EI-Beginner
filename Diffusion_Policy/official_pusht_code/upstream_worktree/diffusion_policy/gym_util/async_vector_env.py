...
class AsyncVectorEnv(VectorEnv):
    ...
    def reset_async(self, seed=None, return_info=False, options=None):
        ...
        # Backward / forward compatibility with newer Gym signatures.
        # The Push-T runner seeds each worker explicitly before reset, so
        # return_info and options are ignored here. We still accept them to
        # avoid signature mismatches with newer Gym versions.
        if seed is not None:
            self.seed(seed)
